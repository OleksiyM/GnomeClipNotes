#!/usr/bin/env bash
# Guided bootstrap. Keep execution inside the complete function for curl | bash.
gcn_install() (
    set -euo pipefail
        ...
        case "$1" in
            ...
            --help)
                printf '%s\n' 'GnomeClipNotes install/update' \
                    'Usage: bash guided-install.sh [--version v1.0.0] [--app-dir PATH] [--yes] [--install-deps] [--require-provenance]' \
...
host = (values.get('ID', ''), values.get('VERSION_ID', ''), platform.machine())
release = {
...
if release is None:
    raise SystemExit('No release package configured for this platform: ' + '-'.join(host))
print(release)
PY
    )
